...
async function referenceStageInner(f) {
  const health = await agent(
    ...
    'Grade status: "fresh" = every surface has usable coverage newer than ' + REFRESH_STALE_DAYS + ' days at the BEST RUNG this machine can actually reach today (for online-only families, a solid official set — App Store/docs/video frames — counts as fresh even though it is staged); "stale" = usable but older than that for a live-capturable tier; "weak" = coverage misses a surface entirely, or a concretely BETTER rung is reachable right now (e.g. real pixels exist upstream but only marketing shots are on disk); "missing" = no usable references.\n' +
    'ACQUISITION IS EXPENSIVE — do not send the loop shopping for material equivalent to what is already on disk. Only grade below "fresh" when this run could actually acquire something better than the current set.\n' +
...
function critiqueStage(preScore, f) {
  ...
  return agent(
    ...
    'ALSO read the newest docs/quality-audit-*.md — it carries the owner/auditor grade and named gaps for this family; close its listed gaps before hunting new ones, and if your judgment of the family disagrees with its grade, say so in referenceQuality.\n' +
    'REFERENCE SANITY CHECK first: confirm the newest dated ref matches the family\'s best tier in reference/sources.json (a native-local family judged from web/marketing refs = wrong target). If the ref is stale or wrong-tier, report gap #1 as "reference stale/wrong tier: <what to recapture>" instead of polishing toward the wrong target.\n' +
...
if (changedFamilies.length === 0) {
  ...
  pr = await agent(
    ...
    '5. gh pr create --base main --head "$BRANCH" --title "Fidelity pass: <families> (<date>)" --body-file <a file you write>. BODY: Read docs/loop-pr-template.md FIRST and follow its fidelity skeleton EXACTLY — TL;DR bullets up top, one ### section per surface with a one-line what-changed, Before/After images STACKED vertically via markdown image syntax ![label](raw url) (NEVER a table, NEVER a bare URL — bare URLs render as links, not images), all scores/change-lists/skip-reasons inside <details> blocks. Media raw URLs: https://raw.githubusercontent.com/conmeara/ui-backlot/"$BRANCH"/reports/fidelity/pr-media/"$BRANCH"/<id>-before.png (same for -after).\n' +
