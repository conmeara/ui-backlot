...
const GOTCHAS = [
  ...
  '3. FIDELITY-FIRST: recreate the real app as closely as possible — match real fonts, glyphs, logos, colors, and spacing exactly (tracking a real font or icon to nail it is fine, docs/asset-policy.md). Build in editable HTML/CSS/SVG because surfaces must animate for video, not because of any asset rule. Only hard line: do not commit the owner\'s private/logged-in captures (use synthetic demo content in surfaces). SCOPE: do not edit styles/backlot-foundation.css, tools/, runtime/, or files outside your scope list.',
